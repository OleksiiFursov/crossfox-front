...
const each = (query: string, callback: Function, parent = d) => {
...
export function DragDropContext(props: IDragDropContext) {
	...
	useEffect(() => {
		for (const id of selected) {
			if (!selections.items.includes(id)) {
				addSelection(find(id));
			}
		}
		for (const id of selections.items) {
			if (!selected.includes(id)) {
				removeSelection(find(id));
			}
		}
	}, [selected]);
	...
	const HandleMouseDown:MouseEventHandler = (e:MouseEvent) => {
		const el = (e.target.closest('[data-draggable="item"]') || e.target) as HTMLElement;
	...
	const HandleMouseUp:MouseEventHandler = e => {
		const el = e.target.closest('[data-draggable="item"]') || e.target;
	...
	const HandleDragStart = e => {
		...
		if (selections.owner !== e.target.parentNode) { //closest
			e.preventDefault();
			return;
		}
